...
from eval.ruler.data.template import Templates
import nltk
try:
...
MODEL2TEMPLATE = {
    "meta-llama/Llama-2-7b-chat-hf": "meta-chat",
    "meta-llama/Meta-Llama-3-8B-Instruct": "meta-llama3",
    ...
    "mistralai/Ministral-8B-Instruct-2410": "base",
}
...
def main():
    ...
    # Customize the settings for different models
    model_name = args.tokenizer_path.split('/')[-1]
    args.save_dir = os.path.join(args.save_dir, model_name, f"{args.max_seq_length}")
    Path(args.save_dir).mkdir(parents=True, exist_ok=True)
    args.model_template_type = MODEL2TEMPLATE.get(args.tokenizer_path, "base")

    try:
        module = importlib.import_module(f"eval.ruler.data.{args.benchmark}.constants")
    except ImportError:
    ...
    if args.random_seed is None:
        args.random_seed = int(time.time() * 1e6) % (2**32 - 1) ^ os.getpid()
