...
def get_score_one_longbook_sum_eng(pred: str, label: str) -> float:
    rouge = Rouge()
    try:
        score = rouge.get_scores([pred], [label], avg=True)
    except:  # noqa
        return 0.0
...
def compute_scores(preds_path, data_name: str, max_seq_length=-1):
...
def cal_score(save_path):
    ...
    for filename in tqdm(all_files, desc="Evaluation..."):
        ...
        score = compute_scores(file_path, dataset)
        scores[dataset] = round(score, 2)
